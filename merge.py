...
def merge(left: Union[list, tuple], right: Union[list, tuple]) -> Union[list, tuple]:
    ...
    if len(left) == 0:
        ...
        left_gen, right_gen = iter(left), iter(right)
        left_item, right_item = next(left_gen), next(right_gen)

        while True:
            if left_item <= right_item:
                result.append(left_item)
                try:
                    left_item = next(left_gen)
                ...
                result.append(right_item)
                try:
                    right_item = next(right_gen)
                except StopIteration:
                    result.append(left_item)
                    break
        for item in left_gen:
            result.append(item)
        for item in right_gen:
            result.append(item)
